# -*- coding: utf-8 -*-
"""
太空杀：三阵营对抗 公测3.0 —— 自动化模拟与平衡性测试
裁判(确定性) + 玩家AI(策略/信息博弈) + 数据统计
参考：太空杀_公测3.0_规则.md（公测3.0 规则正文，8000+字消歧版）

设计原则：
- 裁判逻辑 100% 确定性，严格按照 公测3.0 步骤顺序结算。
- 玩家决策带随机性与策略模型，且禁止"上帝视角"——只能使用自己已知信息 + 公开公告。
- 重点实现高水平玩家的【私聊与讨论博弈】：配对公告作为推理素材、私聊三层信息交换、
  白天讨论节奏、投票追责机制、信息迷雾。
- 觉醒方向额度采用【永久占位制】：额度不随异形死亡释放，转化也不释放旧方向，
  一个异形可同时在多个方向留下额度占位；目标方向额度须<2方可占用。

公测3.0 关键变化（规则↔实现逐条核对后的修正，规则文本为准）：
- R1 救援医师救援额度定版为全局3次（初始1 + 第2/4夜各+1）——2.0 规则文本写5、代码为3不一致；公测3.0 平衡调整定为3
- R11 初始倒计时 20→21 昼夜（公测3.0 平衡调整，与救援削弱同批定版）
- R2 警察巡逻/开枪互斥改为【同夜二选一】（2.0 代码误为巡逻过一次永久不能再开枪）
- R3 外星人双刀觉醒时点提前到第6夜起（规则"第6夜起可选觉醒"；2.0 代码误为第7夜）
- R4 外星人第10夜夜晚免疫恢复（公测1.2.1-E 加强：仅1v1触发、全局限1次、未消耗可带入夜晚交锋）
- R5 结茧护盾不再每夜重置（规则：护盾上限1、直到被打破；2.0 代码每夜清空导致结茧无意义）
- R6 夜晚交锋中外星人可以击杀异形（规则"持续战斗至一方全灭"；2.0 代码外星人无法反击异形）
- R7 验票官紧急会议公开自身身份（规则"公开身份"；2.0 代码未公告编号与职业）
- R8 普通船员排除信息池不再错误排除查验者自身职业（规则：排除任意非目标真实职业）
- R9 移除伪装(disguised)死代码（规则无伪装能力，stealth 为行为隐蔽风格）
- R10 40夜强制判平改为 200 夜模拟护栏（规则：已解除夜数上限，护栏仅防死循环，不计入规则）
- AI 3.0 升级：确定性击杀/协同 scapegoat 投票/证据分级采信/外星人建图-收割时序/追加维修经济

运行：python 太空杀_公测3.0_模拟代码.py
"""
...
# ============================ 常量 ============================
TOTAL = 15
INIT_COUNTDOWN = 21.0  # 公测3.0 平衡定版：初始倒计时上调至 21 昼夜（救援医师同步削弱至3次）

# 人类职业数量
HUMAN_ROLES = (['普通船员'] * 4 + ['工程师', '警察', '生化医师', '救援医师',
               '神探', '保镖', '验票官'])

# 停摆阈值（净破坏量，公测2.0：3.0/6.0/9.0）
...
class Player:
    def __init__(self, pid, camp, role):
        ...
        self.police_patrol_used = False  # 巡逻全局仅1次
        self.patrolled_tonight = False   # 公测3.0-R2：本夜已巡逻（巡逻/开枪同夜互斥）
...
class Game:
    def __init__(self, rng, game_id, strategy='high'):
        ...
        self.human_countdown_dead = False  # 9.0 触发 → 倒计时胜利永久失效（公测2.0）
        self.over = False
        self.winner = None
        self.end_night = None
        ...
        self.night_war_no_kill = 0   # 公测2.0：交锋连续无减员夜数（兜底终止用）
        ...
        # 策略矩阵模式钩子（外部脚本可覆盖，默认 std；不影响默认高水平模拟）
        self.human_mode = 'std'       # std / passive(信息沉淀·神探不跳) / aggro(激进广播·神探必跳)
        self.alien_mode = 'std'       # std / sabotage(偏破坏流) / mix(破坏混合) / stealth(伪装欺骗)。
        # 公测2.0 默认 std(感染击杀流)：核心数值修正后矩阵实测 std(39.6%) > mix(39.0%)，感染→击杀流仍最优
        self.foreigner_mode = 'std'   # std / hunter(更激进双刀收割)
        # 公测2.0：异形行动选择子场景统计（劣势 vs 优势局面的策略选择分布）
        ...
        # 公测3.0-R4：外星人第10夜夜晚免疫——全局仅1次的加强额度是否已发放
        self.foreigner_night10_granted = False
        # 本夜异形协同 scapegoat（白天协同投票目标，公测3.0-AI）
        self._alien_scapegoat = None

        # ===== 公测2.0 信息密度量化（不动规则，只打标统计）=====
        self.chat_events = []          # 每次私聊事件: (night, a, b)，用于有效私聊率追踪
        self.chat_effective = set()    # 判定为"有效"的私聊事件索引（事件后48h内行为变动）
        self.accuse_claims = []        # 每条公开指控: (night, speaker, target, grade, is_fake)
        self.accuse_grade_dist = Counter()  # 指控具体度0-3级分布
        self.fake_accuse_count = 0     # 异形/外星人制造的虚假指控次数
        self.accuse_hit = defaultdict(list)  # 按指控级别的驱逐正确性: grade -> [0/1,...]
        self.idi_by_night = {}         # 逐夜信息密度指数 IDI
        self._chat_effective_cnt = 0   # 累计有效私聊次数
        self._chat_total_cnt = 0       # 累计私聊总次数
    ...
    def _setup_roles(self):
        ...
        for p in players:
            if p.role == '警察':
                p.bullets = 2
            if p.role == '工程师':
                p.engineer_night1_immune = True  # 公测2.0 4.2：第1夜被动全能免疫
            if p.role == '生化医师':
                p.doctor_rescue = 1
                p.doctor_treat = 1
                p.has_antibody = True  # 自身免疫感染
            if p.role == '救援医师':
                p.doctor_rescue = 1  # 公测3.0 平衡定版：救援额度全局3次（初始1 + 第2/4夜各+1）
                p.doctor_treat = 1
            if p.role == '外星人':
                p.night_immune = 1
    ...
    def apply_infection(self, target_id, src_id):
        ...
        if t.dying:
            return 'invalid'
        # 公测3.0 规则消歧：结茧护盾只抵挡"伤害"（濒死类攻击），不抵挡感染（感染非伤害）。
        if t.immune > 0:
            return 'blocked'
        if t.infection >= 1:
    ...
    def run_night(self):
        ...
        # 公测3.0-R5：结茧护盾【不】随夜重置（规则：护盾上限1、直到被打破或再次结茧）
        for p in self.players:
            p.immune = 0
            p.disguised = False
            p.suppressed = False
            p.patrolled_tonight = False  # 公测3.0-R2：巡逻/开枪同夜互斥，每晚重置
            if p.silent > 0:
                p.silent -= 1
        # 公测3.0-R4：外星人第10夜夜晚免疫（公测1.2.1-E 加强，规则文本为准）——
        # 仅 1v1（全局存活恰好2人：外星人对单对手）时触发，全局仅1次；
        # 未被消耗则永久保留，可带入夜晚交锋模式继续使用。
        if self.night >= 10 and not self.foreigner_night10_granted:
            if len(self.alive_players()) == 2 and len(self.alive_foreigners()) == 1:
                for fp in self.alive_foreigners():
                    fp.night_immune += 1
                self.foreigner_night10_granted = True
        ...
        # ---- 步骤4a 维修 ----
        # 夜晚交锋模式：维修/破坏/停摆全部冻结（规则要求），仅保留夜晚行动与死亡公告
        if not self.night_war:
            self.step4a_repair()
        ...
        # ---- 步骤8 医生 ----
        # 医生额度逐夜增长（生化医师治疗初始1，第2/3/4夜各+1=4；救援医师救援初始1，第2/4夜各+1=全局3次）
        ...
        # 记录快照
        # 公测2.0 人类反破坏流：检测净破坏量加速（连续2夜净增≥1.0）→ 触发集中维修模式
    ...
    def step0_private_chat(self):
        ...
        for a, b in pairs:
            ...
            # 公测2.0：记录私聊事件（供有效私聊率追踪）
            self.chat_events.append([self.night, a, b, False])  # [night, a, b, effective]
            self._chat_total_cnt += 1
        # 公告
        if pairs:
    ...
    def _do_transfer(self, p):
        if self.strategy == 'random':
            ...
            # 公测3.0-AI 转职决策（按局势缺口补位）：
            # 1) 双医师均已死亡且无临时医生 → 临时医生（救援/治疗缺口优先）
            # 2) 异形已暴露≥2 且人类无枪 → 武装船员（白天清场需要武力兜底）
            # 3) 倒计时吃紧(≤12) 或 净破坏>3 → 加速工程师
            # 4) 其余：武装/临时医生对半
            doctors_alive = [q for q in self.alive_players()
                             if q.role in ('生化医师', '救援医师', '临时医生')]
            humans_with_gun = [q for q in self.alive_players()
                               if q.role in ('警察', '武装船员') and q.bullets > 0]
            exposed_aliens = len(self.alien_sabotage_exposed) + getattr(self, 'alien_public_exposed_count', 0)
            if len(doctors_alive) == 0:
                d = '临时医生'
            elif exposed_aliens >= 2 and not humans_with_gun:
                d = '武装船员'
            elif self.countdown < 10 or self.net_sabotage > 3:
                d = '加速工程师'
    ...
    def _alien_disadvantaged(self):
        """异形劣势局面评估（仅用异形可见信息）：
        公测2.0 动态破坏触发的前提判断——当异形处于劣势时，"破坏推停摆拖残局"成为
    ...
    def _alien_maybe_awaken(self, p):
        ...
        if self.strategy == 'random':
            ...
            if not preferred:
                preferred = avail
            # 公测2.0 动态分支：异形劣势局面 → 破坏觉醒优先（赌停摆锁死人类倒计时，拖入残局）；
            # 正常局面维持最优转化流（感染开局→击杀收割）。
            # 策略矩阵 alien_mode：sabotage 偏破坏流 / mix 平衡破坏 / stealth 同 std（靠行动层结茧）
            # mix = "少数派破坏"：团队内至多 1 只异形走破坏方向（1破坏+2感染/击杀分工），
            # 若队友已占破坏方向，则本只异形觉醒时不再优先破坏（避免全员破坏流）。
            if self.alien_mode == 'sabotage':
                order = ['破坏', '感染', '击杀']
            elif self.alien_mode == 'mix':
                if '破坏' in teammates_dirs:
                    order = ['感染', '击杀', '破坏']   # 已有1只破坏，其余走感染/击杀
                else:
                    order = ['破坏', '感染', '击杀']   # 尚无破坏分工，允许1只走破坏
            elif self.alien_mode == 'stealth':
                order = ['感染', '击杀', '破坏']
            elif self.dyn_sab_enabled and self._alien_disadvantaged():
                order = ['破坏', '感染', '击杀']
            else:
                order = ['感染', '击杀', '破坏']
    ...
    def _transform(self, p, nd):
        # 异形转化【永久占位制·规则5.3】：
        # 1. 目标方向占位数必须 < 2（awak_quota[nd] > 0 表示未满）
        # 2. 曾占用过目标方向 → 仅切换方向，不增加占位（不扣额度）
        # 3. 从未占用 → 占位数+1，获得标记
    ...
    def step1_foreigner_check(self):
        """公测2.0 6.2：外星人独立查验技能（每夜可用，获知一人真实身份，无视隐藏）。
    ...
    def step2_check_patrol(self):
        ...
        for p in self.alive_players():
            if p.role == '神探' and p.alive and not p.dying:
                ...
                if tgt is not None:
                    t = self.players[tgt]
                    # 公测3.0-R9：移除伪装(disguised)分支——规则无伪装能力，查验恒得真实身份
        ...
        for p in self.alive_players():
            if p.role == '普通船员' and p.alive and not p.dying:
                ...
                if tgt is not None:
                    ...
                    # 公测3.0-R9：移除伪装分支，查验恒得真实结果
                    # 公测3.0 4.1：首次查验获得"该玩家不是某职业"的排除信息（保证不排除真实职业）。
                    # 公测3.0-R8：排除池=全部人类职业中除目标真实职业外的任意一个
                    #（2.0 误排除查验者自身职业，使信息无谓变弱；目标为异形时任意人类职业均为真命题）
        ...
        for p in self.alive_players():
            if p.role == '警察' and p.alive and not p.dying and not p.police_patrol_used:
                ...
                if self.night <= 3 and self._police_will_patrol(p):
                    ...
                    p.patrolled_tonight = True  # 公测3.0-R2：本夜已巡逻，同夜不可开枪
        ...
        for p in self.alive_players():
            ...
            if p.role == '工程师':
                ...
                if self._will_repair(p):
                    ...
                    # 公测3.0-AI 追加维修经济：倒计时充裕(>15)时保留额度，吃紧时才追加（全局3次稀缺资源）
                    if self.engineer_append_left > 0 and self.countdown <= 15:
    ...
    def step6_guns(self):
        ...
        for p in self.alive_players():
            ...
            if p.silent > 0 or p.suppressed:
                continue  # 公测2.0 2.1/0.5：沉默/感染抑制者无法执行夜间技能
            # 公测3.0-R2：巡逻与开枪互斥改为【同夜二选一】（2.0 误为巡逻过永久禁枪）
    ...
    def step7_alien_action(self):
        ...
        for p in self.alive_aliens():
            ...
            if act == '感染':
                ...
                hits = 1
                # 注释⑫：击杀觉醒"偶数夜出刀+1"，且觉醒后第二个夜晚才能触发（觉醒当夜不触发）；
                # 若该夜为奇数则顺延至下一偶数夜。
                if p.awak_dir == '击杀' and self.night % 2 == 0 \
                        and self.night > getattr(p, 'awak_night', 0):
                    hits = 2
                for _ in range(hits):
                    tgt = self.ai_pick_kill_target(p)
                    if tgt is None:
    ...
    def _camp_cn(self, camp):
        return {'human': '人类', 'alien': '异形', 'foreigner': '外星人', 'draw': '平局'}[camp]
            ...
            for p in self.alive_players():
                ...
                if p.role == '验票官' and p.alive and not p.dying and not p.emergency_used:
                    ...
                    if do:
                        ...
                        # 公测3.0-R7：规则"公开身份"——紧急会议公告含验票官编号与职业
                        self.announce_msg("验票官发动紧急会议！（%d号 公开身份：验票官；追加投票清场）" % p.id)
    ...
    def run_daytime(self, emergency=False):
        if self.night_war and not emergency:
            return  # 1.4模式无白天
        # 公测2.0 3.1：任意异形累计破坏3次后，下一白天公开其编号与身份
        for p in self.alive_aliens():
        ...
        # 公测2.0：有效私聊率判定 + 指控命中统计 + IDI 逐夜指数
        self._eval_info_metrics()
        # 胜利检查
        self.check_win('day_vote')

    def _claim_grade(self, p, tgt):
        """公测2.0 指控具体度分级（0-3级，纯打标不改博弈逻辑）：
        0级：无实质内容泛泛怀疑（suspicion 低于阈值或随机泛怀疑）
        1级：引用了可验证行为异常（维修暴露 exposed / 私聊频繁）
        2级：引用了交叉验证信息（多人指控同一目标）
        3级：引用了确证信息（神探查验 / 普通船员累计2次锁定阵营 / 自身 known 确认）
        返回 grade, is_fake。"""
        is_fake = False
        # 异形/外星人的指控天然是"信息污染"（无确证，泛泛指控）
        if p.is_alien() or p.is_foreigner():
            is_fake = True
            return 0, is_fake
        grade = 0
        # 3级：自身确证信息（神探查验 / 普通船员锁定）
        if tgt in p.known and p.known[tgt].get('camp') == 'alien':
            grade = max(grade, 3)
        # 3级：普通船员累计2次查验锁定
        ...
        if len(self.players[tgt].chat_partners) >= 2:
            grade = max(grade, 1)
        # 2级：交叉验证（≥2名不同玩家指控同一目标）
        acc_tgt = set()
        for q in self.alive_players():
            for (sp, tg) in self.belief[q.id]['accuse_log']:
                if tg == tgt:
                    acc_tgt.add(sp)
        if len(acc_tgt) >= 2:
            grade = max(grade, 2)
        return grade, is_fake
    ...
    def _day_discussion(self):
        ...
        - 公测2.0：每条指控同步打"具体度分级(0-3)"标签，供信息密度量化。"""
        # 可信广播：神探/船员锁定的异形
        trusted_claims = []  # (speaker, target, grade)
        for p in self.alive_players():
            if self.strategy != 'high':
                break
            if p.is_alien() or p.is_foreigner():
            ...
            if p.role == '神探':
                if self.human_mode == 'passive':
                    jump = False  # 信息沉淀：神探永不跳身份
                elif self.human_mode == 'aggro':
                    jump = len(known_alien) >= 1  # 激进广播：神探必跳
                else:
                ...
                if self.human_mode == 'aggro':
                    jump = len(locked) >= 1  # 激进广播：船员必跳
                else:
            ...
            if jump and known_alien:
                ...
                grade, _ = self._claim_grade(p, tgt)
                trusted_claims.append((p.id, tgt, grade))
                self.belief[p.id]['accuse_log'].append((p.id, tgt))
                self.revealed_humans.add(p.id)  # 跳身份者公开暴露自身（异形可据此推断威胁）
                # 公测2.0：记录指控打标
                self.accuse_claims.append((self.night, p.id, tgt, grade, False))
                self.accuse_grade_dist[grade] += 1
        # 可信广播 → 全体人类采信（信息战核心：神探情报公开化）
        for _, tgt, _ in trusted_claims:
            for q in self.alive_humans():
    ...
    def _day_vote(self):
        ...
        for p in self.alive_players():
            ...
            # 公测3.0-AI：异形白天协同投票——把票集中到 scapegoat（公开指控最多的人类），
            # 借人类自己的交叉验证机制放大伤害（≥3人指控同一目标 → 全体怀疑+0.1）。
            if p.is_alien():
                sg = self._alien_scapegoat
                if sg is not None and sg != p.id and self.players[sg].alive \
                        and sg not in mates and self.rng.random() < 0.8:
                    votes[sg] += 1
                    voters[p.id] = sg
                    continue
            ...
            if p.is_foreigner():
                ...
                # 公测3.0-AI：防异形清场——外星人胜利路径是"独存"（人类+异形全灭），
                # 异形人数尚多(≥3)且人类健康时，投已知异形阻止其过早清场（残局渔翁定位）。
                expose = (humans_alive <= 3 and known_alien) or \
                         (aliens_alive <= 2 and known_alien and self.rng.random() < 0.6) or \
                         (known_alien and aliens_alive >= 3 and humans_alive >= 5
            ...
            if not cands:
                continue
            tgt, sus = max(cands, key=lambda x: x[1])
            # 公测3.0-AI：人类证据分级采信——低证据不投/少投（降低误驱好人率）。
            # 最高怀疑≥0.35（含 grade-3 确证广播拉升至 0.9）时正常投票；
            # 否则 50% 跟票（保留白天驱逐压力与信息测试功能，仅削去纯噪音票）。
            if sus < 0.35 and self.rng.random() >= 0.5:
                voters[p.id] = None  # 弃权（证据不足）
                continue
            votes[tgt] += 1
            voters[p.id] = tgt
    ...
    def _eval_info_metrics(self):
        ...
        1. 有效私聊率：私聊事件后48小时内，参与双方是否产生"行为变动"
           （查验目标改变 / 投票目标改变 / 公开发言引用该私聊）→ 标记为有效。
        2. 指控命中统计：按指控具体度(0-3)统计被驱逐目标的正确性。
        3. IDI 逐夜指数 = 有效私聊次数×1 + 公开指控总具体度×1 − 虚假指控次数×1。"""
        # —— 1. 有效私聊率判定 ——
        # 有效 = 私聊后48小时内，参与双方中有人对"私聊对象"产生针对性的行为变动：
        #   (a) 公开指控了私聊对象（引用私聊线索转化为公开证据）
        #   (b) 对该私聊对象的怀疑值发生实质变动（±0.15以上，说明私聊内容影响了判断）
        for ev in self.chat_events:
            if ev[3]:  # 已判定
                continue
            night, a, b, _ = ev
            # 48小时内（当前夜距私聊夜 ≤2）
            if self.night - night > 2:
                continue
            a_effective = self._chat_cited(a, b, night) or self._chat_sus_shift(a, b, night)
            b_effective = self._chat_cited(b, a, night) or self._chat_sus_shift(b, a, night)
            if a_effective or b_effective:
                ev[3] = True
                self._chat_effective_cnt += 1
        # 私聊夜已过48h的未判定事件 → 记为无效（不重复统计）
        for ev in self.chat_events:
            if not ev[3] and self.night - ev[0] > 2:
                ev[3] = True  # 锁定为无效（不计数）
        # —— 2. 指控命中统计：本夜被驱逐者若在本夜指控表中，按其等级记录命中/未命中 ——
        # 命中 = 被驱逐者是异形/外星人（正确驱逐）；未命中 = 被驱逐者是人类（误驱）
        ej = self.last_ejected
        if ej is not None:
            ejp = self.players[ej]
            hit = 0 if ejp.is_human() else 1  # 0=未命中(误驱人类), 1=命中(正确驱逐)
            for (cn, sp, tg, grade, _) in self.accuse_claims:
                if cn == self.night and tg == ej:
                    cur = self.accuse_hit[grade]
                    cur.append(hit)
            # 若被驱逐者无本夜指控，记入 grade=0（无依据驱逐）
            if not any(cn == self.night and tg == ej for (cn, sp, tg, grade, _) in self.accuse_claims):
                self.accuse_hit[0].append(hit)
        # —— 3. IDI 逐夜指数 ——
        # 本夜新增有效私聊数（仅计私聊夜==本夜且判定为有效的；48h判定延后计入不重复）
        eff_this = sum(1 for ev in self.chat_events if ev[3] and ev[0] == self.night)
        # 本夜指控总具体度
        grade_sum = sum(g for (cn, _, _, g, _) in self.accuse_claims if cn == self.night)
        # 本夜新增虚假指控数
        fake_this = sum(1 for (cn, _, _, _, isf) in self.accuse_claims
                        if cn == self.night and isf)
        self.idi_by_night[self.night] = (eff_this * 1.0 + grade_sum * 1.0
                                         - fake_this * 1.0)

    def _chat_cited(self, pid, partner_id, since_night):
        """玩家 pid 是否在 since_night 后的白天公开指控了私聊对象（引用私聊线索）。"""
        b = self.belief.get(pid)
        if not b:
            return False
        for (sp, tg) in b['accuse_log']:
            if sp == pid and tg == partner_id:
                return True
        return False

    def _chat_sus_shift(self, pid, partner_id, since_night):
        """玩家 pid 对私聊对象的怀疑值是否发生实质变动（±0.15 以上）。
        通过快照：比较私聊夜的 suspicion 与当前 suspicion（若对象存活）。"""
        if not self.players[partner_id].alive:
            return False
        b = self.belief.get(pid)
        if not b or partner_id not in b['suspicion']:
            return False
        cur = b['suspicion'][partner_id]
        # 私聊事件本身会通过 ai_exchange_chat 改变 suspicion，这里检测的是"后续夜"的进一步变动。
        # 简化：若该玩家在私聊后发起过针对私聊对象的公开指控 → 已由 _chat_cited 覆盖。
        # 兜底：若当前怀疑值>0.85 且私聊对象是异形/外星人（被私聊套话识破）→ 视为有效。
        if cur >= 0.85 and (self.players[partner_id].is_alien()
                            or self.players[partner_id].is_foreigner()):
            return True
        return False
    ...
    def ai_pick_check_target(self, p):
        ...
        if p.role == '普通船员':
            half = [q for q in others if p.crew_checks.get(q.id, 0) == 1]
            if half:
                return self.rng.choice(half).id
        # 未知目标（未被确定性查验过）
        unknown = [q for q in others if q.id not in p.known]
        if not unknown:
            return None  # 公测3.0-R9：全图已知，无查验需求（原伪装复验分支随 R9 移除）
        # 神探：盲搜应均匀扫查未知者（怀疑度在异形隐匿时不可靠），保证以≈异形占比命中
        if p.role == '神探':
            return self.rng.choice(unknown).id
        b = self.belief[p.id]
        # 其余：按怀疑度查验（伪装分支已随 R9 移除）
    ...
    def ai_pick_shoot(self, p):
        ...
        b = self.belief[p.id]
        # 公测3.0-AI：确定性击杀优先——自身查验确证的异形必打（子弹用在确定性情报上）
        known_alien = [q for q in self.alive_players()
                       if q.id != p.id and q.alive and not q.dying
                       and p.known.get(q.id, {}).get('camp') == 'alien']
        if known_alien:
            return known_alien[0].id
        # 仅对已较高怀疑者开枪（避免误杀好人）；3.0 提高阈值至 0.6（子弹稀缺，误杀代价高）
    ...
    def ai_pick_kill_target(self, p):
        ...
        # 公测3.0-R6：夜晚交锋（人类全灭）时外星人的击杀目标包含异形——
        # 规则"持续战斗至一方全灭"，外星人的终局胜利路径是独存（异形也须死）。
        # 公测3.0-AI：外星人全程可选中异形（"1v1 铺路"需要），人类优先级更高时另行排序。
        if p.is_foreigner():
            others = [q for q in self.alive_players() if q.id != p.id]
        else:
        ...
        if self.strategy == 'random':
            return self.rng.choice(
                [q for q in others if not q.is_alien()] if not p.is_foreigner() else others).id
        # 外星人：双刀沉默收割——基于查验建图(known)精准锁定最控制力强的目标
        # 公测3.0-AI：高价值名单加入生化医师（治疗/救援/抗体三重资源点）
        if p.is_foreigner():
            known_hv = [q for q in others if q.id in p.known
                        and p.known[q.id].get('camp') == 'human'
                        and p.known[q.id].get('role') in ('神探', '验票官', '救援医师', '生化医师')]
            if known_hv:
                return known_hv[0].id
            # 公测3.0-AI "1v1 铺路"：人类残局(≤4)且异形≥2时，削减异形至1只——
            # 人类全灭后进入夜晚交锋，1异形vs1外星人三夜未分胜负→外星人胜（规则1.4单挑例外）。
            if len(self.alive_humans()) <= 4 and len(self.alive_aliens()) >= 2:
                alien_tgts = [q for q in others if q.is_alien()]
                if alien_tgts:
                    return self.rng.choice(alien_tgts).id
        threat = []
        for q in others:
            score = 0
            ...
            # 公测3.0-AI：信息枢纽打击——私聊网覆盖广者=信息网节点（配对公告公开可见），
            # 刀信息枢纽可延缓人类情报扩散（维护费用：目标可能是低价值船员，权重保守）
            score += min(1.5, len(q.chat_partners) * 0.25)
        ...
        if p.is_alien() and getattr(self, '_alien_targets_this_night', None):
            ...
            if unpicked:
                threat = unpicked
        # 无明确信号者按随机（不预知角色）
        # 公测3.0：外星人在人类尚存时的随机兜底不指向异形（避免无谓开战），
        # 仅"1v1 铺路"分支与夜晚交锋主动选异形。
        fallback = others
        if p.is_foreigner() and self.alive_humans() and len(self.alive_aliens()) >= 1:
            non_alien = [q for q in others if not q.is_alien()]
            if non_alien:
                fallback = non_alien
        if threat[0][1] == 0:
            return self.rng.choice(fallback).id
    ...
    def plan_night_actions(self):
        ...
        # 公测3.0-AI：异形白天协同 scapegoat——三只异形把公开票集中到同一人类身上。
        # scapegoat 选取只用公开信息：累计被公开指控次数最多的存活非异形玩家
        #（异形自己制造的指控也会计入——它们知道哪些指控是自己人发的，故意叠加火力）。
        acc_cnt = Counter()
        for (cn, sp, tg, grade, _) in self.accuse_claims:
            acc_cnt[tg] += 1
        ...
        for p in aliens_alive:
            if self.strategy == 'random':
                ...
                if dyn_sab:
                    ...
                    # 伪装欺骗：低频行动，更多结茧蓄力（降低暴露、延长信息迷雾）
                    r = self.rng.random()
                    if p.awak_dir == '感染':
                        p._alien_action = '感染' if r < 0.7 else '结茧'
                    elif r < 0.35:
                        p._alien_action = '出刀'
                    elif r < 0.5:
                        p._alien_action = '感染'
        ...
        # 外星人（公测2.0：激进出刀策略，行动优先出刀，第七个晚上起优先觉醒双刀）
        for p in self.alive_foreigners():
            if p.dying:
    ...
    def _foreigner_plan_normal(self, p):
        """外星人3.0 计划：前期查验建图 → 中后期确定性收割。
        - known<3 且 night<6：以查验建图为主（70%），其余出刀（避免无图盲刀误杀外星人威胁评估）。
        - 净破坏≥6 且 人类≤6 且未用破坏：用破坏推 9.0 停摆（倒计时胜利失效对残局外星人有利）。
        - 其余情况出刀（高价值目标由 ai_pick_kill_target 基于 known 精准选择）。"""
        known_cnt = len([v for v in p.known.values()])
        # 破坏时点：9.0 阈值前且人类尚多时推停摆（对残局外星人有利：倒计时胜利失效+交锋冻结维修）
        if p.foreigner_sabotage_count < 1 and self.net_sabotage >= 6.0 \
                and len(self.alive_humans()) <= 6:
            p._foreigner_action = '破坏'
            return
        if known_cnt < 3 and self.night < 6:
            r = self.rng.random()
            if r < 0.7:
                p._foreigner_action = '查验'
            else:
                p._foreigner_action = '击杀'
        else:
            r = self.rng.random()
            if r < 0.8:
                p._foreigner_action = '击杀'
    ...
    def _foreigner_plan(self, p):
        """外星人 3.0 策略：
        - 双刀觉醒时点：第6夜起（规则6.3：第6夜起可选觉醒；公测3.0-R3 修正 2.0 误写的第7夜）。
        - 觉醒后每夜自主选择双刀/单刀+查验（规则6.3"以代码实测为准，非强制每夜双刀"）。
        - 收割时序：known 中存在 高价值人类(神探/验票官/救援医师) 且 (night≥8 或 人类≤5) → 双刀精准沉默收割；
        - 夜晚交锋（人类全灭）：主动击杀异形（公测3.0-R6 修复外星人无法反击的问题）。"""
        humans_alive = len(self.alive_humans())
        # 夜晚交锋：无人类可杀，目标转为异形（最终独存胜利）
        if humans_alive == 0:
            p._foreigner_action = '击杀'
            return
        if not getattr(p, 'fore_double_awakened', False):
            if self.night >= 6:  # 公测3.0-R3：规则"第6夜起可选觉醒"
                # 觉醒决策：已有高价值图源或中后期必觉醒（外星人胜负手在于双刀收割节奏）
                if self.foreigner_mode == 'hunter':
                    awaken_p = 0.95
                else:
                    awaken_p = 0.9
            ...
            # 已觉醒：有高价值目标且时机成熟 → 双刀沉默收割；否则单刀+查验补图
            known_hv = [t for t, v in p.known.items()
                        if v.get('camp') == 'human'
                        and v.get('role') in ('神探', '验票官', '救援医师')
                        and self.players[t].alive and not self.players[t].dying]
            if known_hv and (self.night >= 8 or humans_alive <= 5):
                p._foreigner_action = '双刀'
            elif self.rng.random() < 0.15 and len(p.known) < 8:
                p._foreigner_action = '查验'  # 持续补图
            else:
                p._foreigner_action = '击杀'
...
# ============================ 模拟驱动 ============================
def simulate_one(rng, game_id, strategy='high', modes=None):
    g = Game(rng, game_id, strategy)
    if modes:
        g.human_mode = modes.get('human', 'std')
        g.alien_mode = modes.get('alien', 'std')
        g.foreigner_mode = modes.get('foreigner', 'std')
    # 公测3.0 规则：已解除夜数上限，游戏持续到一方胜利。
    # R10：max_nights=200 仅为模拟护栏（防极端死循环），护栏判平不计入规则与报告口径（实测触发率≈0）。
    max_nights = 200
    while not g.over and g.night < max_nights:
        ...
        g.check_win('loop')
    if not g.over:
        # 公测3.0-R10：200 夜模拟护栏（非规则），触发即判平并保留统计可见性
...
class Stats:
    def __init__(self):
        ...
        # 公测2.0 子场景统计：异形优势/劣势局面的策略选择分布与胜率
        ...
        # 公测2.0 信息密度量化收集
        self.chat_total = 0          # 私聊总次数
        self.chat_effective = 0      # 有效私聊次数
        self.accuse_grade_dist = Counter()   # 指控具体度0-3级分布
        self.fake_accuse_total = 0   # 虚假指控总数
        self.accuse_hit_by_grade = defaultdict(list)  # grade -> [0/1 命中序列]
        self.idi_by_night = defaultdict(lambda: [0.0, 0])  # night -> [sum, cnt]
    ...
    def collect(self, g):
        ...
        # 公测2.0 信息密度量化收集
        self.chat_total += g._chat_total_cnt
        self.chat_effective += g._chat_effective_cnt
        self.fake_accuse_total += g.fake_accuse_count
        for grade, cnt in g.accuse_grade_dist.items():
            self.accuse_grade_dist[grade] += cnt
        for grade, hits in g.accuse_hit.items():
            self.accuse_hit_by_grade[grade].extend(hits)
        for night, idi in g.idi_by_night.items():
            self.idi_by_night[night][0] += idi
            self.idi_by_night[night][1] += 1
        # 阶段
        ln = g.end_night
...
def run_simulation(n_games, strategy, seed=20260820, modes=None):
    rng = random.Random(seed)
    stats = Stats()
    for i in range(n_games):
        g = simulate_one(rng, i, strategy, modes=modes)
        stats.collect(g)
    return stats
...
def write_report(stats, strategy_name, path, compare=None):
    n = stats.n
    L = []
    L.append("# 太空杀 公测3.0 自动化模拟报告 — %s" % strategy_name)
    L.append("")
    L.append("模拟局数：**%d** 局  | 规则版本：公测3.0（裁判确定性结算）｜ 停摆阈值 3.0/6.0/9.0" % n)
    L.append("说明：觉醒方向额度采用【永久占位制】（不随死亡/转化释放，转化仅占用目标方向额度）；")
    L.append("公测3.0 规则核对修正：R1 救援医师3次(平衡定版) / R2 巡逻开枪同夜互斥 / R3 双刀第6夜觉醒 / R4 外星人第10夜免疫(1v1) /")
    L.append("R5 结茧护盾跨夜持续 / R6 交锋外星人可击杀异形 / R7 紧急会议公开身份 / R8-R9 排除信息与伪装清理 / R10 解除夜数上限 / R11 初始倒计时21。")
    L.append("公测3.0 AI 升级：确定性击杀 / 异形协同 scapegoat 投票 / 人类证据分级采信 / 外星人建图-收割时序 / 追加维修经济 / 转职补位。")
    L.append("")
    # ① 总体
    L.append("## ① 总体结果")
    L.append("")
    ...
    L.append("| 平局（200夜护栏判平） | %s |" % fmt_pct(stats.wins.get('draw', 0), n))
    ...
    L.append("| 平局（200夜护栏判平，非规则） | %d | %s |" % (stats.wins.get('draw', 0), fmt_pct(stats.wins.get('draw', 0), n)))
    L.append("")
    L.append("> 公测3.0：已解除夜数上限（规则文本），游戏进行至一方胜利；200 夜仅为模拟护栏（实测触发率≈0）。")
    # ② 停摆
    L.append("## ② 停摆系统")
    L.append("")
    L.append("| 指标 | 结果 |")
    ...
    L.append("")
    # ②.5 信息密度量化（公测2.0）
    L.append("## ②.5 信息密度量化（有效私聊率 / 指控具体度 / IDI）")
    L.append("")
    L.append("### 有效私聊率")
    L.append("")
    if stats.chat_total:
        L.append("- 私聊总次数：**%d** 次；有效私聊（48h内产生针对私聊对象的行为变动）：**%d** 次。" % (
            stats.chat_total, stats.chat_effective))
        L.append("- **有效私聊率：%.1f%%**" % (100.0 * stats.chat_effective / stats.chat_total))
    else:
        L.append("- 本组模拟未产生私聊。")
    L.append("")
    L.append("### 指控具体度分级与驱逐正确率")
    L.append("")
    L.append("| 指控级别 | 数量 | 占比 | 驱逐正确率 |")
    L.append("|----------|------|------|------------|")
    tot_acc = sum(stats.accuse_grade_dist.values())
    for g in [0, 1, 2, 3]:
        c = stats.accuse_grade_dist.get(g, 0)
        hits = stats.accuse_hit_by_grade.get(g, [])
        cr = ("%.1f%%" % (100.0 * sum(hits) / len(hits))) if hits else "-"
        L.append("| %d级 | %d | %s | %s |" % (g, c, fmt_pct(c, tot_acc) if tot_acc else "-", cr))
    L.append("")
    L.append("> 判据：0级(泛泛怀疑)→3级(确证信息)的驱逐正确率应逐级提升；若2级与3级接近，"
             "说明AI未区分证据强弱、在跟风投票，需提高3级指控采信权重。")
    L.append("")
    L.append("### 信息密度指数 IDI（逐夜均值）")
    L.append("")
    L.append("| 夜 | IDI | 样本局 |")
    L.append("|----|-----|--------|")
    for night in sorted(stats.idi_by_night):
        ssum, cnt = stats.idi_by_night[night]
        L.append("| %d | %.1f | %d |" % (night, ssum / cnt, cnt))
    L.append("")
    L.append("> IDI = 本夜有效私聊数×1 + 公开指控总具体度×1 − 虚假指控数×1。"
             "逐夜曲线反映每夜信息战活跃度：前中期信息激烈（跳身份/交叉验证），后期随存活者减少自然回落。")
    L.append("")
    # ③ 觉醒
    L.append("## ③ 觉醒系统")
    L.append("")
    ...
    if compare is not None:
        ...
        # 公测2.0 子场景统计：异形优势/劣势局面的策略选择分布与胜率
        L.append("### 异形子场景策略分析（劣势 vs 优势局面，公测2.0 动态破坏触发验证）")
        L.append("")
...
def write_report_html(stats, strategy_name, path, compare=None):
    ...
    s.append('</div>')
    if stats.wins.get('draw', 0):
        s.append("<p class='note'>其中 200 夜护栏判平：%d 局（%.1f%%），三方均不得胜。</p>" % (
            stats.wins.get('draw', 0), 100.0 * stats.wins.get('draw', 0) / n))
    ...
    rows.append(["平局（200夜护栏判平）", "%d" % stats.wins.get('draw', 0), fmt_pct(stats.wins.get('draw', 0), n)])
    ...
    s = []
    if stats.chat_total:
        s.append("<p><b>有效私聊率</b>：私聊总次数 <b>%d</b>，有效私聊（48h内对私聊对象产生行为变动）<b>%d</b> 次 → "
                 "<b>%.1f%%</b></p>" % (stats.chat_total, stats.chat_effective,
                                        100.0 * stats.chat_effective / stats.chat_total))
    else:
        s.append("<p>本组模拟未产生私聊。</p>")
    s.append("<h3>指控具体度分级与驱逐正确率</h3>")
    tot_acc = sum(stats.accuse_grade_dist.values())
    rows = []
    for g in [0, 1, 2, 3]:
        c = stats.accuse_grade_dist.get(g, 0)
        hits = stats.accuse_hit_by_grade.get(g, [])
        cr = ("%.1f%%" % (100.0 * sum(hits) / len(hits))) if hits else "-"
        rows.append(["%d级" % g, "%d" % c, (fmt_pct(c, tot_acc) if tot_acc else "-"), cr])
    s.append(_html_table(["指控级别", "数量", "占比", "驱逐正确率"], rows))
    s.append("<p class='note'>判据：0级(泛泛怀疑)→3级(确证信息)的驱逐正确率应逐级提升；"
             "若2级与3级接近，说明AI未区分证据强弱、在跟风投票，需提高3级指控采信权重。</p>")
    s.append("<h3>信息密度指数 IDI（逐夜均值）</h3>")
    rows = []
    for night in sorted(stats.idi_by_night):
        ssum, cnt = stats.idi_by_night[night]
        rows.append(["%d" % night, "%.1f" % (ssum / cnt), "%d" % cnt])
    s.append(_html_table(["夜", "IDI", "样本局"], rows))
    s.append("<p class='note'>IDI = 本夜有效私聊数×1 + 公开指控总具体度×1 − 虚假指控数×1。"
             "逐夜曲线反映每夜信息战活跃度。</p>")
    sections.append(("②.5 信息密度量化", "\n".join(s)))
    ...
    if compare is not None:
        ...
        def _act_share_html(act_counter, act_key):
            tot = sum(act_counter.values())
            return (fmt_pct(act_counter.get(act_key, 0), tot) if tot else "-")
        s.append("<h3>异形子场景策略分析（劣势 vs 优势局面，公测2.0 动态破坏触发验证）</h3>")
        rows = [
...
<title>太空杀 公测3.0 模拟报告 — @@TITLE@@</title>
<style>
...
<body>
<header><h1>太空杀 公测3.0 自动化模拟报告 — @@TITLE@@</h1>
<p>模拟局数：@@N@@ 局 ｜ 规则版本：公测3.0（裁判确定性结算）｜ 觉醒方向额度采用永久占位制（转化不释放）｜ 停摆阈值 3.0/6.0/9.0｜ 规则核对修正 R1-R10 + AI 3.0 升级</p></header>
...
if __name__ == '__main__':
    ...
    import time
    t0 = time.time()
    # 公测3.0：输出到脚本所在目录（2.0 误用硬编码本机绝对路径）
    out_dir = os.path.dirname(os.path.abspath(__file__))
    def _out(name):
        return os.path.join(out_dir, name)

    N = 5000  # 公测3.0 高水平报告：5000局
    print("运行 %d 局高水平模拟 (公测3.0)..." % N)
    stats_high = run_simulation(N, 'high', seed=20260821)
    report_path = _out("太空杀_公测3.0_高水平报告.md")

    # 随机基线（用于数值vs推理平衡对照）
    print("运行 %d 局随机基线..." % N)
    stats_rand = run_simulation(N, 'random', seed=99)
    report_path2 = _out("太空杀_公测3.0_随机基线报告.md")
    write_report(stats_rand, "随机基线玩家", report_path2)
    print("随机基线报告：", report_path2)

    # 高水平报告（附带与随机基线的对比）
    strategy_label = "高水平玩家（AI 3.0：确定性击杀·协同投票·建图收割）"
    write_report(stats_high, strategy_label, report_path, compare=stats_rand)
    print("报告已生成：", report_path)

    # HTML 报告（提示词重要指标可视化）
    html_path = _out("太空杀_公测3.0_高水平报告.html")
    write_report_html(stats_high, strategy_label, html_path, compare=stats_rand)
