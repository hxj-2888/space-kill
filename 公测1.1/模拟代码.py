# -*- coding: utf-8 -*-
"""
太空杀：三阵营对抗 公测1.1 —— 自动化模拟与平衡性测试
裁判(确定性) + 玩家AI(策略/信息博弈) + 数据统计
参考：rules_ob11.md (公测1.1 规则正文) / sim_prompt.md (模拟提示词)

设计原则：
- 裁判逻辑 100% 确定性，严格按照 公测1.1 步骤顺序结算。
...
- 公测1.1 关键变化：异形动态破坏触发（已 A/B 实测为净负面，默认关闭切回静态感染→击杀流）；人类反破坏流（净破坏加速→集中维修）；统计口径修复（end_reason 带阵营前缀）。
...
class Player:
    def __init__(self, pid, camp, role):
        ...
        self.silenced_once = False  # 公测1.1 7.4：是否已触发过沉默（全局仅1次）
...
class Game:
    def __init__(self, rng, game_id, strategy='high'):
        ...
        self.human_countdown_dead = False  # 9.0 触发 → 倒计时胜利永久失效（公测1.1）
        self.over = False
        self.winner = None
        self.end_night = None
        ...
        self.night_war_no_kill = 0   # 公测1.1：交锋连续无减员夜数（兜底终止用）
        ...
        # 公测1.1：异形行动选择子场景统计（劣势 vs 优势局面的策略选择分布）
        ...
        # 外星人破坏减速剩余夜数（公测1.1 6.5，全局状态）
        self.fore_slow_remain = 0
        # 工程师追加维修全局次数剩余（公测1.1 3.1：共3次）
        self.engineer_append_left = 3
        # 已因累计破坏3次而暴露编号的异形（公测1.1 3.1，只暴露一次）
        self.alien_sabotage_exposed = set()

        # ===== 公测1.1 信息密度量化（不动规则，只打标统计）=====
    ...
    def _setup_roles(self):
        ...
        for p in players:
            if p.role == '警察':
                p.bullets = 2
            if p.role == '工程师':
                p.engineer_night1_immune = True  # 公测1.1 4.2：第1夜被动全能免疫
            if p.role == '生化医师':
                p.doctor_rescue = 1
                p.doctor_treat = 1
    ...
    def apply_harm(self, target_id, cause):
        ...
        if t.immune > 0:
            t.immune -= 1
            return 'immune'
        # 公测1.1 6.6：外星人夜晚免疫（自动触发，当夜剩余时间免疫所有伤害）。
    ...
    def run_night(self):
        ...
        # 记录快照
        # 公测1.1 人类反破坏流：检测净破坏量加速（连续2夜净增≥1.0）→ 触发集中维修模式
        ...
        for p in self.alive_players():
            if p.infection == 1 and p.infect_suppress_quota > 0:
                ...
                if use:
                    ...
                    # 公测1.1 0.5：当夜失去所有主动行动能力（不进入沉默，不影响白天投票）
                    p.suppressed = True
    ...
    def _alien_disadvantaged(self):
        """异形劣势局面评估（仅用异形可见信息）：
        公测1.1 动态破坏触发的前提判断——当异形处于劣势时，"破坏推停摆拖残局"成为
    ...
    def _alien_maybe_awaken(self, p):
        ...
        if self.strategy == 'random':
            ...
            if not preferred:
                preferred = avail
            # 公测1.1 动态分支：异形劣势局面 → 破坏觉醒优先（赌停摆锁死人类倒计时，拖入残局）；
    ...
    def step2_check_patrol(self):
        ...
        for p in self.alive_players():
            if p.role == '神探' and p.alive and not p.dying:
                if p.silent > 0:
                    continue  # 公测1.1 2.1：沉默者次夜无法执行夜间技能
            ...
            if p.role == '普通船员' and p.alive and not p.dying:
                ...
                if tgt is not None:
                    ...
                    if t.disguised:
                        ...
                        # 公测1.1 4.1：首次查验获得"该玩家不是某职业"的排除信息（保证不排除真实职业）
    ...
    def step4a_repair(self):
        ...
        for p in self.alive_players():
            ...
            if p.role == '工程师':
                ...
                if self._will_repair(p):
                    ...
                    eng_repaired = True
                    p.repair_count += 1
                    # 追加维修（公测1.1 3.1：全局共3次，须在基础维修后立即追加，不可单独使用）
                ...
                if self._crew_will_repair(p):
                    # 协助维修：与查验互斥（公测1.1 3.1：-0.20 - 0.05×(X-1)）
        ...
        # 维修暴露标记（公测1.1 3.2：仅向所有异形和外星人公布其编号与具体职业，不公开给人类）
        for p in self.alive_players():
    ...
    def _crew_will_repair(self, p):
        ...
        if not crew_list:
            return False
        # 公测1.1 反破坏流：净破坏量连续加速 → 人类集中维修反制（牺牲部分查验/投票效率）
            ...
            for p in self.alive_players():
                if p.role == '验票官' and p.alive and not p.dying and not p.emergency_used:
                    ...
                    if do:
                        ...
                        if self.over:
                            return
                        self._emergency_triggered = True  # 公测1.1 2.2：紧急会议跳过步骤11
                        break
    ...
    def run_daytime(self, emergency=False):
        if self.night_war and not emergency:
            return  # 1.4模式无白天
        # 公测1.1 3.1：任意异形累计破坏3次后，下一白天公开其编号与身份
        for p in self.alive_aliens():
    ...
    def _day_discussion(self):
        ...
        - 公测1.1：每条指控同步打"具体度分级(0-3)"标签，供信息密度量化。"""
        # 可信广播：神探/船员锁定的异形
    ...
    def _day_vote(self):
        ...
        for p in self.alive_players():
            ...
            if p.silent > 0:
                voters[p.id] = None  # 公测1.1 2.1：沉默者次日白天投票强制弃权
                continue
            b = self.belief[p.id]
    ...
    def ai_pick_infect(self, p):
        ...
        # 公测1.1 5.4：异形感染自选目标；基础1~2名，感染觉醒后2~3名（数量只由觉醒状态决定）
        if p.awak_dir == '感染':
    ...
    def plan_night_actions(self):
        ...
        for p in aliens_alive:
            if self.strategy == 'random':
                ...
                # 公测1.1 动态破坏触发：异形劣势局面 → 切破坏优先（赌停摆锁死人类倒计时，拖入残局）。
        ...
        # 外星人（公测1.1：查验/击杀/破坏三选一，双刀第6夜后可选）
        for p in self.alive_foreigners():
            if p.dying:
...
class Stats:
    def __init__(self):
        ...
        # 公测1.1 子场景统计：异形优势/劣势局面的策略选择分布与胜率
...
def write_report(stats, strategy_name, path, compare=None):
    n = stats.n
    L = []
    L.append("# 太空杀 公测1.1 自动化模拟报告 — %s" % strategy_name)
    L.append("")
    L.append("模拟局数：**%d** 局  | 规则版本：公测1.1（裁判确定性结算）｜ 停摆阈值 3.0/6.0/9.0" % n)
    L.append("说明：觉醒方向额度采用【永久占位制】（不随死亡/转化释放，转化仅占用目标方向额度）；")
    L.append("公测1.1 关键机制：人类反破坏流（净破坏加速→集中维修）+ 统计口径修复（end_reason 带阵营前缀，两表可对账）。")
    ...
    if compare is not None:
        ...
        # 公测1.1 子场景统计：异形优势/劣势局面的策略选择分布与胜率
        L.append("### 异形子场景策略分析（劣势 vs 优势局面，公测1.1 动态破坏触发验证）")
        L.append("")
...
def write_report_html(stats, strategy_name, path, compare=None):
    ...
    if compare is not None:
        ...
        def _act_share_html(act_counter, act_key):
            tot = sum(act_counter.values())
            return (fmt_pct(act_counter.get(act_key, 0), tot) if tot else "-")
        s.append("<h3>异形子场景策略分析（劣势 vs 优势局面，公测1.1 动态破坏触发验证）</h3>")
        rows = [
...
<body>
<header><h1>太空杀 公测1.1 自动化模拟报告 — @@TITLE@@</h1>
<p>模拟局数：@@N@@ 局 ｜ 规则版本：公测1.1（裁判确定性结算）｜ 觉醒方向额度采用永久占位制（转化不释放）｜ 停摆阈值 3.0/6.0/9.0｜ 反破坏流（动态破坏触发默认关闭）</p></header>
...
if __name__ == '__main__':
    ...
    t0 = time.time()
    N = 1000
    print("运行 %d 局高水平模拟 (公测1.1)..." % N)
    stats_high = run_simulation(N, 'high', seed=20260821)
    report_path = "c:/Users/ASUS/CodeBuddy/20260820234047/report_high_1000_ob11.md"

    # 随机基线（用于数值vs推理平衡对照）
    print("运行 %d 局随机基线..." % N)
    stats_rand = run_simulation(N, 'random', seed=99)
    report_path2 = "c:/Users/ASUS/CodeBuddy/20260820234047/report_random_1000_ob11.md"
    ...
    # HTML 报告（提示词重要指标可视化）
    html_path = "c:/Users/ASUS/CodeBuddy/20260820234047/report_high_1000_ob11.html"
